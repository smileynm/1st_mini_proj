...
LibrarySystem::LibrarySystem() {
    ...
    // 관리자 객체들은 멤버 변수로 선언되었으므로 LibrarySystem 객체 생성 시 자동으로 생성됩니다.
    // 여기서 초기 데이터 로딩을 할 수도 있습니다. (main에서 하거나 여기서 하거나 선택 가능)
}

// 소멸자
LibrarySystem::~LibrarySystem() {
    std::cout << "Library System shutting down." << std::endl;
    // 관리자 객체들은 LibrarySystem 객체 소멸 시 자동으로 소멸됩니다.
    // 여기서 최종 데이터 저장을 할 수도 있습니다. (main에서 하거나 여기서 하거나 선택 가능)
}
...
void LibrarySystem::run() {
    ...
    while (choice != 11) { // 11번 메뉴가 종료라고 가정
        ...
        // 실제 구현에서는 run 함수 내에서 입력 루프를 돌리는 것이 더 일반적입니다.
...
// 사용자 입력 처리 (run 함수 내에서 직접 처리하는 방식으로 변경)
void LibrarySystem::processUserInput() {
    // 이 함수는 이제 run 함수 내에서 직접 입력 처리를 하므로 필요 없을 수 있습니다.
    // 만약 복잡한 입력 유효성 검사 등을 분리하고 싶다면 사용할 수 있습니다.
}
...
void LibrarySystem::handleExit() {
    ...
    // run 함수의 while 루프 조건(choice != 11)에 의해 종료됩니다.
    // 여기서 추가적인 정리 작업이 필요하다면 수행할 수 있습니다.
}
